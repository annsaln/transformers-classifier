...
def argparser():
    ...
    ap.add_argument('--treshold', metavar='FLOAT', type=float, default=None, 
                    help='fixed treshold for multilabel prediction')
...
def main(argv):
    ...
    if options.load_model is not None:
        ...
        if options.treshold is None:
            f1, th, dev_pred = optimize_threshold(classifier, train_X, train_Y, dev_X, dev_Y, options)
        else:
            f1, th, dev_pred = test_treshold(classifier, dev_X, dev_Y, treshold=options.treshold)
        ...
        if options.test is not None:
            ...
            if options.treshold is None:
                ...
                test_f1, test_th, test_pred = test_treshold(classifier, test_X, test_Y, treshold=options.treshold, save_pred_to=options.save_predictions)
